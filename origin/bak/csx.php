				...
				if(preg_match($tokenRegex, substr($this->string, $offset), $matches)) {
					...
					if($tokenType == $lastTokenPatternKey) {
						if($offset == strlen($this->string)) {
							...
							die('Symbol not allowed: '.substr($this->mString, $offset, 1));
						}
					}
				}
			}
...
class Parser {
	...
	function parse($level, $tree, &$scope) {
		...
		foreach($tree as $object) {
			...
			switch($object['type']) {
				...
				case 'object':
					$this->parse($level+1, $children, $scope[$name]);
					break;
					
					
				case 'variable':
					$variable = $this->compute($children, $scope[$name]);
					$scope[$name] = $variable;
					break;
				...
				case 'call':
					$scope[$name] = $this->call($children, $scope[$name]);
					break;
